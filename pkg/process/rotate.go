...
import (
	...
	util "github.com/KoyomiKun/sup/utils/signal"
)
	...
	for _, fi := range fis {
		...
		if now.Sub(backupNow) > w.maxAge {
			...
			if err := os.Remove(filename); err != nil {
				log.Errorf("remove %s: %s", filename, err)
			} else {
				log.Infof("deleted aged log %s", fi.Name())
			}
		}
	}
}
...
func (w *FileWriter) gzip(srcFilename string) {
	...
	if err != nil {
		log.Errorf("gzip open src file %s: %s", srcFilename, err)
		return
	}
	dstFilename := srcFilename + ".gz"
	dst, err := os.OpenFile(dstFilename, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		log.Errorf("gzip open dst file %s: %s", dstFilename, err)
		return
	}
	if _, err = w.gzipCopyClose(src, dst); err != nil {
		log.Errorf("gzip file %s: %s", dstFilename, err)
		return
	}
	if err := os.Remove(srcFilename); err != nil {
		log.Errorf("remove file %s: %s", srcFilename, err)
		return
	}
}
...
func (w *FileWriter) cleanExtraBackups() {
	...
	for _, fi := range fis[:len(fis)-w.maxBackups] {
		...
		if err := os.Remove(filepath.Join(dir, name)); err != nil {
			log.Errorf("remove backup file %s: %s", name, err)
		}
	}
}
...
func (w *FileWriter) parseTimeFromBackup(filename string) time.Time {
	...
	if len(m) != 3 {
		log.Errorf("invalid backup filename format: %s", filename)
